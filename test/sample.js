...
* to an Amazon Cognito User Pool's Fido2 authentication endpoint. It validates the
* requests, determines the user handle and name for the user, and performs
* the appropriate action based on the request path parameter.
* 
* * Starting a new authenticator registration
* * Completing the new authenticator registration
* * Listing
* authenticators for a user
* * Deleting an authenticator
* * Updating an authenticator
* 
* It returns a response with a status code and body containing the
* result of the action, or an error message if there is an
* issue.
* 
* @param { object } event - The `event` parameter is an object
* that contains information about the incoming request. It is used to retrieve
* information about the request, such as the request path, query string parameters,
* and JSON body.
* 
* In this function, `event` is used to extract information from the request
* context, such as the user's sub claim, email, phone number, and other
* details. The `event.requestContext.authorizer.jwt.claims` property is used to access the user's claims, which
* are stored in the JWT token.
* 
* The `event` object is also used to access other information about the
* request, such as the request path and query string parameters. For example,
* `event.pathParameters.fido2path` is used to retrieve the path parameter `fido2path` from the request
* path.
* 
* Overall, the `event` parameter is an important input to the function, as
* it provides the necessary information to process the incoming request and retrieve
* the user's details.
* 
* @returns { object } - Based on the provided function, the output
* returned by the function will be a JSON object with the following
* properties:
* 
* * `statusCode`: 200 (OK) or 400 (Bad Request) or 500 (Internal Server
* Error)
* * `body`: a JSON object containing the response data
* * `headers`: an object
* containing the HTTP headers
* 
* The function handles several different HTTP requests and returns different responses based
* on the request path parameters.
* 
* * `/register-authenticator/start`: Returns an object with `statusCode` set to 200 and `body`
* set to the options for the new authenticator registration. The `headers` object
* will contain the HTTP headers.
* * `/register-authenticator/complete`: Returns an object with `statusCode` set
* to 200 and `body` set to the stored credential for the new
* authenticator registration. The `headers` object will contain the HTTP headers.
* * `/authenticators/list`: Returns
* an object with `statusCode` set to 200 and `body` set to a
* JSON object containing the list of authenticators for the user. The `headers`
* object will contain the HTTP headers.
*/
